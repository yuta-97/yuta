...
결과적으로 전체 테스트 실행 시간을 약 40% 단축할 수 있었고, 세션 만료나 로그인 실패로 인한 테스트 불안정성도 크게 줄었습니다.`}
          ...
          <StoryCard
            ...
            content={`치과 차트 시스템에서는 X-ray 이미지, 동의서 PDF 등 다양한 파일을 업로드하고 다운로드하는 기능이 많이 쓰입니다. 브라우저의 파일 다이얼로그는 일반적인 방법으로는 자동화하기 어려웠습니다.
      ...
      <ProjectSection title="성과">
        <div className="space-y-4">
          <StoryCard
            ...
            content={`수동 테스트 2시간 → 자동화 테스트 20분으로 단축했습니다.
              QA팀은 자동화된 회귀 테스트 대신 새로운 기능의 탐색적 테스트에 집중할 수 있게 되었습니다.
              수동 테스트로 인해 발생 할 수 있는 휴먼 에러를 완전히 제거하고, 버그를 조기에 발견할 수 있게 되었습니다.

              E2E 테스트는 핵심 시나리로 위주로 작성하는것이 효율적이라는 것을 배웠고, 테스트를 염두해 두지 않은 설계는 오히려 유지보수를 어렵게 한다는걸 느꼈습니다.
              모든 것을 테스트 할 필요는 없기 때문에 적절한 테스트 커버리지의 균형이 중요하다는 생각이 들었습니다.
              `}
          />
        </div>
      </ProjectSection>
